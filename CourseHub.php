...
class CourseHub
{
    ...
    // course published in the "offre de formation en libre accès"
    const PUBLISH_PUBLISHED = 0;

    // course published in the "offre de parcours à déployer"
...
FROM {".CourseHub::TABLE_COURSE."} WHERE deleted=0 AND publish=?".$search_where;
    ...
    function shareCourse($courseid,$userid = null) {
        ...
        if ( !$this->backupCourse($courseid, $publishFolder, $publishFilename) || !($fileexist = file_exists($publishFilepath) )) {
            if (!$fileexist){
                CourseHub::log('Backup failed : File not found',$ls);
            } else {
                CourseHub::log('Backup failed : Return false',$ls);
            }
    ...
    function fullRestoreCourse($hubcourseid,$categoryid,$fullname,$shortname,$visible=true) {
        ...
        if (!$fp->extract_to_pathname($publishFilepath, $CFG->tempdir.'/backup/'. $backupfilebase)){
            CourseHub::log('Backup extraction failed : ##'.$publishFilepath.'## => ##'.$CFG->tempdir.'/backup/'. $backupfilebase.'##',$ls);
            return false;
        }
